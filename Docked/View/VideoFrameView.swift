...
//  Docked
//
//  A thin rounded screen bezel drawn around the floating video. The opening
//  has the same rounded corners as an iOS PiP window, so the border hugs it.
//  Until a video is parked there the opening shows faint "no-signal" colour
//  bars behind the "drag your video here" prompt.
//
...
struct VideoFrameView: View {
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            ...
            hintView
                .frame(width: max(0, hole.width - 14), height: max(0, hole.height - 14))
    ...
    private func draw(_ ctx: GraphicsContext, size: CGSize) {
        ...
        let bw = hole.width / CGFloat(barColors.count)
        for (i, col) in barColors.enumerated() {
            let bar = CGRect(x: hole.minX + CGFloat(i) * bw, y: hole.minY, width: bw + 1, height: hole.height)
            screen.fill(Path(bar), with: .color(col.opacity(0.15)))
        }
        var y = hole.minY
        while y < hole.maxY {
            screen.fill(Path(CGRect(x: hole.minX, y: y, width: hole.width, height: 1)),
